...
function MoodboardConfirmModal({ magazineId, onClose, onComplete }: ModalProps) {
    ...
    const handleConfirm = async () => {
        ...
        setIsConfirmed(true)
        try {
            await createMoodboard(magazineId)
            onComplete()
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative z-10 w-120 h-53 rounded-2xl bg-gray-500 px-8 py-8 flex flex-col justify-between shadow-[0_4px_8px_rgba(0,0,0,0.12),0_16px_32px_rgba(0,0,0,0.20)]">
                <div>
                    <h2 className="mb-3 font-semibold20 text-white">무드보드를 변경하시겠습니까?</h2>
